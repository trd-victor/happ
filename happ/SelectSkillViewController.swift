...
class SelectSkillViewController: UIViewController {
    ...
    func addSubSkill(title: String, children: NSArray){
        ...
        let subTitle: UILabel = UILabel(frame: CGRect(x: 0.0, y: CGFloat(height), width: self.view.layer.frame.size.width, height: 38.0))
        ...
        for (value) in children {
            if let data = value as? NSDictionary {
                let skillBox: UIView = UIView(frame: CGRect(x: 0.0, y: CGFloat(height), width: self.view.layer.frame.size.width, height: 48))
...
//        let titlestr = config.translate("title_notification")
        ...
        let navItem = UINavigationItem(title: "Select skill")
        let btnBack = UIBarButtonItem(image: UIImage(named: "Image"), style: .Plain, target: self, action: Selector("backToRegist"))
        ...
        let saveBtn = UIBarButtonItem(title: "Save", style: .Plain, target: self, action: "saveSkill")
